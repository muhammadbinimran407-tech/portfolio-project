...
.success-alert{background:rgba(94,234,212,.12);border:1px solid rgba(94,234,212,.36);color:#5EEAD4;padding:14px 16px;border-radius:16px;margin-bottom:18px;}
...
<section class="page-header">
  ...
  <div class="wrap">
    <div class="breadcrumb"><a href="{{ route('index') }}">Home</a> / Contact</div>
    <h1>Let's Build Something Reliable</h1>
    <p>Have a project in mind? Send the details — I usually reply within a day.</p>
  </div>
</section>
    ...
    <div>
      <div class="glass contact-info-card" data-aos="fade-right">
        ...
        <div class="contact-item">
          ...
          <div><div class="lbl">PHONE</div><div class="val">+92 3092885538</div></div>
        </div>
        <div class="contact-item">
          <div class="ic"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 12-9 12s-9-5-9-12a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></div>
      ...
      <form id="contactForm" action="{{ route('contact.store') }}" method="POST">
        @csrf
        @if($errors->any())
          <div class="glass" style="margin-bottom:18px;padding:14px 16px;border-color:var(--danger);color:var(--danger);">
            Please fix the highlighted fields below before sending.
          </div>
        @elseif(session('success') || isset($success))
          <div class="glass success-alert">
            {{ session('success') ?? $success }}
          </div>
        @endif
        <div class="field{{ $errors->has('name') ? ' error' : '' }}" id="fieldName">
        ...
        <div class="field{{ $errors->has('email') ? ' error' : '' }}" id="fieldEmail">
          <label for="email">EMAIL</label>
          <input type="email" id="email" name="email" placeholder="you@company.com" value="{{ old('email') }}">
          @error('email')<div class="err-msg">{{ $message }}</div>@enderror
        </div>
        <div class="field{{ $errors->has('subject') ? ' error' : '' }}" id="fieldSubject">
          <label for="subject">SUBJECT</label>
          <input type="text" id="subject" name="subject" placeholder="What is this regarding?" value="{{ old('subject') }}">
          @error('subject')<div class="err-msg">{{ $message }}</div>@enderror
        </div>
        <div class="field{{ $errors->has('message') ? ' error' : '' }}" id="fieldMessage">
          <label for="message">MESSAGE</label>
          <textarea id="message" name="message" placeholder="Tell me about your project...">{{ old('message') }}</textarea>
          @error('message')<div class="err-msg">{{ $message }}</div>@enderror
        </div>
        <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:13px;">Send Message</button>
      </form>
  </div>
</section>
...
<nav class="bottom-nav">
  <a href="{{ route('index') }}" class="bn-item">
      ...
      <a href="{{ route('contact') }}" class="bn-item active">
