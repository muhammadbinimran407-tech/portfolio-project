...
<title>Skills — {{ \App\Support\Settings::get('site_name') }}</title>
<meta name="description" content="Technical skills of {{ \App\Support\Settings::get('site_name') }} — Laravel, Go, PHP, JavaScript and more.">
...
<nav id="nav">
  <div class="wrap row">
    ...
    <ul class="nav-links desktop-only">
      ...
      <li><a href="{{ route('skills') }}" class="active-link">Skills</a></li>
...
<section class="page-header">
  ...
  <div class="wrap">
    <div class="breadcrumb"><a href="{{ route('index') }}">Home</a> / Skills</div>
    <h1>Skills &amp; Stack</h1>
    <p>The tools I build with daily, and how deep each one runs.</p>
  </div>
</section>

<section id="skills" style="padding-top:20px;">
  <div class="wrap">
    <div class="stack-panel" data-aos="fade-up">
      <div class="stack-panel-head"><span>primary</span><span>core</span></div>
      <div class="skills-grid">
        @forelse(($skills ?? []) as $skill)
        <div class="skill-row">
          <div class="skill-top">
            <span class="name">{{ $skill->name ?? 'Skill' }}</span>
            <span class="pct">{{ $skill->pct ?? $skill->proficiency ?? '0' }}%</span>
          </div>
          <div class="skill-bar"><i data-w="{{ $skill->pct ?? $skill->proficiency ?? 0 }}"></i></div>
          @if($skill->group || $skill->category)
          <span class="skill-tag">{{ '#' . str_replace(' ', ' #', $skill->group ?? $skill->category) }}</span>
          @endif
        </div>
        @empty
        <p style="grid-column: 1 / -1; text-align: center; color: var(--text-muted);">No skills yet. Add some from the admin panel.</p>
        @endforelse
      </div>
    </div>
  </div>
...
<section id="featured-work" style="padding-top:40px;">
  <div class="wrap">
    <div class="section-head">
      <span class="eyebrow">featured work</span>
      <h2>Built with this stack</h2>
      <p>A few recent projects that use the tools above — head to <a href="{{ route('projects') }}" style="color:var(--accent);">Work</a> for the full list.</p>
    </div>
    <div class="projects-grid">
      ...
      @empty
      <p style="grid-column: 1 / -1; text-align: center; color: var(--text-muted);">No featured projects yet. Mark one as featured in the admin panel.</p>
      @endforelse
    </div>
  </div>
</section>
...
const skillObserver = new IntersectionObserver(entries=>{
  entries.forEach(entry=>{
    if(entry.isIntersecting){
      entry.target.querySelectorAll('.skill-bar i').forEach(bar=>{ bar.style.width = bar.dataset.w + '%'; });
      skillObserver.unobserve(entry.target);
    }
  });
},{threshold:0.3});
document.querySelectorAll('.stack-panel').forEach(p=>skillObserver.observe(p));
