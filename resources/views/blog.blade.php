...
<title>Blog — {{ \App\Support\Settings::get('site_name') }}</title>
<meta name="description" content="Articles on Laravel, Go, APIs and backend engineering by {{ \App\Support\Settings::get('site_name') }}.">
...
<nav id="nav">
  <div class="wrap row">
    ...
    <ul class="nav-links desktop-only">
      ...
      <li><a href="{{ route('blog') }}" class="active-link">Blog</a></li>
...
<section class="page-header">
  ...
  <div class="wrap">
    <div class="breadcrumb"><a href="{{ route('index') }}">Home</a> / Blog</div>
    <h1>Blog &amp; Notes</h1>
    <p>Writing on backend engineering, Laravel, Go, and shipping software that lasts.</p>
  </div>
</section>

<section id="blog" style="padding-top:20px;">
  <div class="wrap">
    <div class="blog-toolbar">
      ...
      <div class="filter-tabs" style="margin-bottom:0;">
        ...
        <button class="filter-tab">Laravel</button>
        <button class="filter-tab">Go</button>
        <button class="filter-tab">DevOps</button>
      </div>
    </div>
    <div class="blog-grid">
      @forelse($posts ?? [] as $post)
      <div class="glass blog-card" data-aos="fade-up">
        <div class="blog-thumb">{{ $post->image ?? 'article-cover.png' }}</div>
        <div class="blog-body">
          @if($post->category)
          <span class="blog-cat">{{ $post->category }}</span>
          @endif
          <h3>{{ $post->title ?? 'Untitled' }}</h3>
          <p>{{ $post->excerpt ?? substr($post->content ?? '', 0, 150) ?? '' }}</p>
          <div class="blog-meta">
            @php
              $postDate = $post->published_at ?? $post->date ?? $post->created_at;
              $postDate = $postDate ? (is_string($postDate) ? \Carbon\Carbon::parse($postDate) : $postDate) : null;
            @endphp
            @if($postDate)
            <span>{{ $postDate->format('M j, Y') }}{{ $post->read_time ? ' · ' . $post->read_time . ' min' : '' }}</span>
            @endif
            @if($post->url)
      ...
      @empty
      <p style="grid-column: 1 / -1; text-align: center; color: var(--text-muted);">No posts yet. Add some from the admin panel.</p>
      @endforelse
    </div>
    <div class="tag-row">
      <span class="tag-pill">#laravel</span><span class="tag-pill">#golang</span><span class="tag-pill">#api-design</span>
      <span class="tag-pill">#mysql</span><span class="tag-pill">#docker</span><span class="tag-pill">#performance</span>
    </div>
  </div>
</section>
...
<nav class="bottom-nav">
  <a href="{{ route('index') }}" class="bn-item">
      ...
      <a href="{{ route('blog') }}" class="bn-item active">
